			...
			<form action="./logout" method="post">
				<table class="table table-bordered">
					<tr>
						...
						<td><input type="hidden" name="Id" value="${user.loginId}">
							<input class="btn btn-light" type="submit" value="ログアウト" /></td>
					</tr>
					<tr>
		...
		<div style="width: 40%" class="container padding-y-5 text-center">
			...
			<form action="./bbs" method="post">
				<table class="table">
					...
					<tr>
						<c:if
							test="${requestScope.alert_shout != null && requestScope.alert_shout != ''}">
							<td colspan="2" class="color-error text-left"><c:out
									value="${requestScope.alert_shout}" /></td>
						</c:if>
					</tr>
					<tr>
			...
			<c:forEach var="shout" items="${shouts}">
				<table class="table table-striped table-bordered">
					<tr>
						...
						<c:if test="${shout.loginId eq user.loginId}">
							<td><form action="./DS" method="post">
									<input type="hidden" name="userName" value="${shout.userName}">
									<input type="hidden" name="loginId" value="${shout.loginId}">
