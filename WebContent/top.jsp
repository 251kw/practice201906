	...
	<div class="padding-y-5">
		<div style="width: 40%" class="container padding-y-5">
			...
			<form action="./logout" method="post">
				...
				<a href="userKensaku.jsp"><button class="btn pull-right" type="button">ユーザー検索</button></a>
			</form>
		</div>
	</div>
	<div class="padding-y-5 text-center">
				...
				<table class="table">
					<tr>
						...
						<td><input class="form-control" type="text" name="shout"
							value="" size="60" /><br>
							<h6 class="color-error text-left">最大256文字</h6>
							</td>
							<!-- 叫ぶ内容とuserのloginIdを一緒に送信 -->
							<td>
			...
			<c:forEach var="shout" items="${shouts}">
				<table class="table table-striped table-bordered">
					<tr>
						...
						<span class="${shout.icon} pe-3x pe-va"></span></td>
						<td>${shout.userName}
						<!-- userのuserNameとshoutのuserNameが一緒なら書き込み内容を送信する削除ボタンを表示 -->
						<c:if test="${user.loginId.equals(shout.loginId) }">
						<!-- formの中にいくつもsubmitボタンを作らない。今回のようにボタンが特定の条件だけ表示される場合はformの生成もその条件内に入れる -->
						<form action="SDK" method="post">
						<!-- hiddenで他のユーザーネームやパラメータ渡した方が○？ -->
						...
						<button class="btn pull-right" type="submit" name="shouterWriting" value="${shout.writing }">削除</button>
						</form>
						</c:if>
						</td>
					</tr>
